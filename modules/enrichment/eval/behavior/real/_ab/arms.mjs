 ...
 * baseline below restates the extras bundle's enrichment-row config (cordis patch
 * config override is whole-replace — see @catheadowl/dsh-eval
 * docs/rowconfig.md; the values mirror the extras bundle's own
 * `cordis.patch.yml` enrichment row), and the control arm overrides ONLY
...
/** Loader row id of enrichment in the extras bundle patch. */
export const ENRICHMENT_ROW_ID = 'enrichment'

/** The extras bundle's enrichment-row config (case-level baseline, restated
 * under whole-replace). */
export const ENRICHMENT_ROW_CONFIG = {
...
export function extractMetrics(trace, spec) {
  ...
  const injectionSeen = (trace?.userMessages ?? []).some(
    message => message.source?.plugin === 'enrichment' && message.text.includes('relates:'),
  )
