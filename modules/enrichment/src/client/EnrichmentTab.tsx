...
import type { EnrichmentLocaleKey } from './locales.js'
import css from './EnrichmentTab.module.css'
...
/** Callbacks the plugin binds from the `enrichment` Host Remote. */
export interface EnrichmentTabInjected {
  list: () => Promise<EnrichmentProviderView[]>
  setDisabled: (ids: string[]) => Promise<EnrichmentProviderView[]>
}
...
function EnrichmentTabForm({ t, list, setDisabled }: EnrichmentTabProps) {
  ...
  const toggle = async (provider: EnrichmentProviderView): Promise<void> => {
 ...
 */
function metaLabel(t: (key: EnrichmentLocaleKey) => string, provider: EnrichmentProviderView): string {
...
function tooltipLabel(t: (key: EnrichmentLocaleKey) => string, provider: EnrichmentProviderView): string {
