 ...
 * `enrichment` remote on load and on every switch, and host enforcement
 * reads that mirror.
 */

const STORAGE_KEY = 'dsh.enrichment.disabled'

/** Read the persisted disabled-name list; a missing or malformed value is empty. */
export function loadDisabledProviderNames(): string[] {
  try {
...
export function saveDisabledProviderNames(names: readonly string[]): void {
  try {
