...
interface InternalEnrichmentRegistry extends EnrichmentRegistry {
  register(provider: EnrichmentProvider, kind?: string, subjectOf?: (path: ResolvedPromptPath) => string): () => void
  listEntries(): RegisteredProvider[]
}

export function validateProvider(provider: EnrichmentProvider): void {
  if (!PROVIDER_NAME.test(provider.name)) {
    throw new Error(`enrichment provider name ${JSON.stringify(provider.name)} must match ${PROVIDER_NAME}`)
  }
  if (provider.priority !== undefined && !Number.isFinite(provider.priority)) {
    throw new Error(`enrichment provider ${JSON.stringify(provider.name)} priority must be a finite number`)
  }
  if (provider.timeoutMs !== undefined && (!Number.isSafeInteger(provider.timeoutMs) || provider.timeoutMs <= 0)) {
    throw new Error(`enrichment provider ${JSON.stringify(provider.name)} timeoutMs must be a positive safe integer`)
  }
  if (provider.mode !== undefined && provider.mode !== 'always' && provider.mode !== 'once') {
    throw new Error(`enrichment provider ${JSON.stringify(provider.name)} mode must be 'always' or 'once'`)
  }
  if (provider.description !== undefined && (typeof provider.description !== 'string' || provider.description.trim() === '')) {
    throw new Error(`enrichment provider ${JSON.stringify(provider.name)} description must be a non-empty string`)
  }
  if (provider.sources !== undefined) {
    if (!Array.isArray(provider.sources) || provider.sources.length === 0) {
      throw new Error(`enrichment provider ${JSON.stringify(provider.name)} sources must be a non-empty array`)
    }
    for (const source of provider.sources) {
      if (source !== 'prompt' && source !== 'touch') {
        throw new Error(`enrichment provider ${JSON.stringify(provider.name)} sources entries must be 'prompt' or 'touch'`)
      }
    }
    if (provider.sources.includes('touch') && provider.touchSubjects === undefined) {
      throw new Error(`enrichment provider ${JSON.stringify(provider.name)} declares the touch source without a touchSubjects projection (dead subscription)`)
    }
  }
  if (provider.touchSubjects !== undefined && typeof provider.touchSubjects !== 'function') {
    throw new Error(`enrichment provider ${JSON.stringify(provider.name)} touchSubjects must be a function`)
  }
}

function materializeRelatesProvider(decl: DeclarativeRelatesProvider): { provider: EnrichmentProvider; kind: string; subjectOf?: (path: ResolvedPromptPath) => string } {
  if (typeof decl.kind !== 'string' || decl.kind === '') {
    throw new Error(`enrichment declarative provider ${JSON.stringify(decl.name)} kind must be a non-empty string`)
  }
  ...
  if (mode !== undefined && mode !== 'always') {
    throw new Error(`enrichment declarative provider ${JSON.stringify(decl.name)} mode must be 'always' or omitted (once is the default)`)
  }
  if (decl.subjectOf !== undefined && typeof decl.subjectOf !== 'function') {
    throw new Error(`enrichment declarative provider ${JSON.stringify(decl.name)} subjectOf must be a function`)
  }
  const provider: EnrichmentProvider = {
    ...
    mode: decl.mode ?? 'once',
    run: async (input) => {
      const contributions: RelatesContribution[] = []
...
export function createEnrichmentRegistry(): EnrichmentRegistry {
  let nextOrder = 0
  const byName = new Map<string, RegisteredProvider>()
  const registry: InternalEnrichmentRegistry = {
    register(provider, kind, subjectOf) {
      validateProvider(provider)
      if (byName.has(provider.name)) {
        throw new Error(`enrichment provider ${JSON.stringify(provider.name)} is already registered`)
      }
...
function subscribesPrompt(provider: EnrichmentProvider): boolean {
  return provider.sources?.includes('prompt') ?? true
}

function subscribesTouch(provider: EnrichmentProvider): boolean {
...
type RunnerConfig = Required<Pick<EnrichmentConfig, 'providerTimeoutMs' | 'totalTimeoutMs' | 'renderBudgetChars'>>

export class EnrichmentRunner {
  private readonly registry = createEnrichmentRegistry() as InternalEnrichmentRegistry
  ...
  constructor(config: EnrichmentConfig = {}) {
    ...
    for (const [key, value] of Object.entries(this.config)) {
      if (!Number.isSafeInteger(value) || value <= 0) {
        throw new Error(`enrichment: ${key} must be a positive safe integer`)
      }
    }
  }
  ...
  listEntries(): EnrichmentProviderEntry[] {
  ...
  async run(options: EnrichmentRunOptions): Promise<EnrichmentRunResult> {
    const trace: EnrichmentTraceEvent[] = []
    ...
    for (const [providerIndex, entry] of providers.entries()) {
      ...
      let providerInput: EnrichmentInput = { ...input, paths: effectivePaths }
...
interface ProviderRunResult {
  contributions?: RelatesContribution[]
  event: EnrichmentTraceEvent
}
...
function countItems(contributions: readonly RelatesContribution[]): number {
  return contributions.reduce((total, contribution) => total + contribution.items.length, 0)
}

function dedupePaths(paths: readonly ResolvedPromptPath[], trace: EnrichmentTraceEvent[]): ResolvedPromptPath[] {
  const seen = new Set<string>()
...
function normalizeContributions(
  ...
  contributions: readonly RelatesContribution[],
...
export function renderRelates(relates: readonly RelatesGroup[], budgetChars = DEFAULT_RENDER_BUDGET_CHARS): RenderResult {
