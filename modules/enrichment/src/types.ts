...
export interface RelatesContribution {
  path: string
  items: RelatesItem[]
}
 ...
 */
export type EnrichmentProviderMode = 'always' | 'once'

/** Signal sources a provider consumes; omitted means prompt-only (v0 behavior). */
export type EnrichmentSource = 'prompt' | 'touch'
...
export interface RelatesResolveContext {
  ...
  /** The whole-turn enrichment input (cwd / signal / agent / session / turnId / stepId). */
 ...
 * `EnrichmentProvider` and reuses the shared runner (once ledger, merge/
...
export interface DeclarativeRelatesProvider {
   ...
   * `EnrichmentProvider.sources` — same contract, declarative face.
   */
  sources?: EnrichmentSource[]
  /**
   * Reverse touch projection onto this provider's subjects; see
   * `EnrichmentProvider.touchSubjects` — same contract, declarative face.
   */
  touchSubjects?(touchedPath: string, context: TouchSubjectContext): string[]
  /**
...
export type EnrichmentTraceStatus =
  | 'ok'
  | 'skipped'
  | 'failed'
  | 'timed-out'
 ...
 */
export interface EnrichmentIntrospection {
  name: string
  /** Provider-authored one-liner; absent when the provider declared none. */
  description?: string
  /** Item kind of a declarative provider; imperative providers have none. */
