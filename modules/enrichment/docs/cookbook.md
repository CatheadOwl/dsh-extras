...
想验证你的声明会渲染成什么样：调 `ctx.enrichment.introspect()`（或 remote `enrichment/introspect`）——设置面就是它的投影；headless / eval 侧查 provider 生效状态（`effectiveEnabled` / `disabledBy`）也走这里，字段表见 [contract.md](contract.md)「自省快照」节。
