...
test('matching is case-insensitive: a lowercase query hits a differently-cased candidate', () => {
  assert.deepEqual(suggestPathCandidates(['workunits/enrichment/', 'Inbox/'], 'inbox'), {
    matches: ['Inbox/'],
    total: 1,
  })
})
...
test('matching is case-insensitive: extension stripping and multi-segment queries fold too', () => {
  ...
  assert.deepEqual(suggestPathCandidates(['workunits/enrichment/'], 'Workunits/Enrichment'), {
    matches: ['workunits/enrichment/'],
    total: 1,
  })
})
