// Real composition test: boots enrichment + any_nav over a real
...
test('once-mode breadcrumb injects once, dedupes across turns, and re-arms after compact', async () => {
  ...
  try {
    ...
    // ctx.inject(['enrichment']), which settles off-turn.
    await waitFor(() => ctx.get('enrichment').list().length >= 1)
    ...
    const injections = () => agent.session.snapshotEvents().filter(event =>
      ...
      && event.data.source?.plugin === 'enrichment'
...
test('non-delivery trace events warn; designed degradation stays at debug', async () => {
  ...
  try {
    ...
    await waitFor(() => ctx.get('enrichment').list().length >= 3)
    const ask = () => createUserMessage({
    ...
    assert.ok(
      warns.some(line => line.includes('enrichment: boom-declarer failed: kaboom')),
      `a failed provider must surface at warn, got ${JSON.stringify(warns)}`,
    )
    assert.ok(
      debugs.some(line => line.includes('enrichment: steady-declarer skipped: all paths already injected this session')),
