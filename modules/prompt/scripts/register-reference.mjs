// register face 的 generated API reference:check(root) 供 node:test 消费;
// --write 时原地重生成 docs/register.md 的 generated region。
...
export async function check(packageRoot = root, ts) {
  ...
  return changed
    ? [{
        ...
        remedy: {
          kind: 'manual',
          guidance: 'Run node scripts/register-reference.mjs --write from the prompt module, then review the public contract change.',
        },
      }]
    : []
}
