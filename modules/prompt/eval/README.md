...
| 用例 | 模式 | 验证 |
|---|---|---|
| `behavior/mock/injection-smoke.eval.mjs` | mock(免 key) | 单轮 path mention 恰好注入一条 prompt-middleware `relates:` user/message（断言在 `inspect` hook 读原始 session 事件，故声明 `evidence: 'inspect'`） |
...
| `behavior/real/run-relates-ab.mjs` | real A/B 驱动 | 双臂（`disabledProviders` 差异）× N 次跑上述 case，trace 机械指标 + 臂守卫，产出到 `.runs/relates-ab-*`（设计记录按名引用：relates 行为 A/B 探针原始设计存于开发仓库；框架行为实验面 `defineBehaviorExperiment` 落地后驱动迁移属发布面后续） |
