...
| `behavior/real/run-relates-ab.mjs` | real A/B 驱动 | 双臂（`disabledProviders` 差异）× N 次跑定位 case，trace 机械指标 + 臂守卫，产出到 `.runs/relates-ab-*`（设计记录按名引用：relates 行为 A/B 探针原始设计存于开发仓库） |
