---
description: prompt 模块文档导航入口——注入契约、注册消费面与 parse/tree 两个内嵌库契约文档的索引
---

# prompt · docs

- [why-prompt](why-prompt.md) —— 为什么需要 prompt 模块：宿主 pre-step 的机制事实与注册面/共存纪律价值
- [contract](contract.md) —— 注入契约权威文本（执行模型、once 去重、`registerRelates` 与 `subjectOf`、定序、provider 开关）
- [register](register.md) —— 插件消费面（`@catheadowl/dsh-extras/prompt/register`）注册示例与公共 API reference
- [cookbook](cookbook.md) —— 用法指南（cookbook）：touch lane 三种消费角色、配对 provider 完整模式（稳态 resolve / 自编辑对账 / 镜像对齐）与语义速查
- [parse](parse.md) —— prompt-parse 解析库契约
- [tree](tree.md) —— workspace-tree 目录枚举库契约（vendored `ignore`）
- [development](development.md) —— 维护指南：本机命令、组合测试前置
