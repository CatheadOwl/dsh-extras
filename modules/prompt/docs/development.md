...
pnpm run test:prompt    # 库(parse/tree) + 框架 + wire + client-storage + 组合测试（见下）
```

## 组合测试（host 接线条件运行）

两套组合测试已挂在 `test:prompt` 末尾，经包根 `scripts/run-composition.mjs`（不随包发布，名称引用）条件运行：
...
- **前置**：host 源码 junction + 宿主检出已 build（接线方式见 [../../docs/development.md](../../docs/development.md) 的 peer junction 接线节）。
