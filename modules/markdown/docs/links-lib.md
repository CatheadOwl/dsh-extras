...
| `resolve.ts` | 自写（语义对齐上游） | 解析缝之上：`extractReferences`（AST 提取 link/image/definition）、`resolveReference`（逐引用解析）、`canonicalPath`（归责规范路径：绝对、`/` 分隔、lexical）、`checkRepository`（整仓校验，可选 `include` 谓词缝）；跳过 `//`/`/`/scheme，`#frag` 解析到源文件自身 |
...
- **fork（copy = fork，上游改动需手工重拷 + 重测）**：`markdown.ts`（fork 自上游
  ...
  （fork 自 `deepseek-harness/scripts/verify-md-links.ts` 的
  `githubSlug`/`documentAnchors`/`anchorCache`）。不得 import 上游源码（反转依赖，被否决的 A3）。
- **自写（无宿主等价物，本仓测试负责）**：`resolve.ts`（宿主的 `findViolations` 是 gate 编排、
...
- `@types/mdast` 在 `dependencies`（非 dev）：`MarkdownDestinationNode = Extract<Nodes, …>`
  把 mdast `Nodes` 类型泄漏进公共 `.d.ts`，消费者需要它。
- `@types/node` 在 `devDependencies`：`anchors.ts` 读 `node:fs`，只编译期需要；
  tsconfig 用宿主约定 `"types": ["node"]`（宿主 `tsconfig.base.json` 同款）。
