...
（`@deepseek-ai/*` → harness；mdast 解析栈 → host `.pnpm`）。开发期解析说明
（嵌套仓库内不要 `pnpm install`）见 [links-lib.md](links-lib.md)。

## E2E 测试（headless behavior harness）

`eval/` 目录用共享 eval 框架（devDep `@catheadowl/dsh-eval`，`dsh-eval` bin）覆盖
工具内三层 fallback（deterministic rebase / skip 非阻塞 / conflict 阻塞整单）。
case 清单与 out-of-scope 见 `eval/README.md`（行为 eval 目录，不随包发布）。
