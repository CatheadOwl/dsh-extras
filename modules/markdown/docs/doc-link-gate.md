...
两种形态同一份 `check` 实现、语义互斥：插件级已注册时根 `gates.yml` 不再保留 `doc-link` 条目（撞名会以 `gates-config` 冲突 gate 暴露）。
...
安装 extras 后：故意写一条断链 → 轮末被 steer 定位 + rationale + remedy；修复后静默通过。手动面：`/gates` 聚合中出现 `doc-link`（插件注册）。
