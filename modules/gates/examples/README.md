...
非空 frontmatter `description`」检查的 module 形态参考实现。Lineage：仓库级 `gates.yml` 条目的
数据面 `scripts/md-metadata-lib.mjs`（defer-bypass probe 的观测对象），
2026-09-02 检查升插件级（`@catheadowl/dsh-extras` markdown 模块
