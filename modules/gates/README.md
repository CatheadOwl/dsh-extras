...
- `doc-link`（**插件级**，`dsh-plugin-dev/md-links-gates` 注册）：Markdown 链接完整性，
...
# 前置：先 build md-links-gates（W10 用例 import 其 lib/gate-check.js 构建产物）
...
- 用户开关是**全局偏好**（按 gate id × trigger 生效，不按工作区分），持久化在**浏览器
  ...
  （全关语义）在读取时自动迁移为两维全关。
- Web 配置页（Settings → Plugins → Gates）与 host 插件代码同生命周期：修改后需重建（`tsc` + `tsdown`）并重启 host 才会被加载（新增 `dsh.client` 声明后的首次扫描在启动时进行）。
