...
dsh 已原生暴露 Cordis DI、`agent/turn-stopping` 驱动、会话 cwd、取消
`signal`、工具事件流与轮次预算——任何插件都可以直连这些低层能力。gates 的
定位是**能力补足，不是独立产品，也不是宿主内核改造**：把这些低层能力收敛成
一套稳定的 gate 契约（注册、执行、预算、反馈、修复、手动运行、仓库级配置、
插件级软依赖注册），使注册方插件不必各自重复实现触发、超时、节流与失败语义。
所谓 native-like，指直接使用 dsh 的类型化扩展点与 Cordis DI，而非走
CC/Codex hook 方言桥（与宿主 hooks 子系统的边界见上）。若宿主未来出现同类
内置注册表，本插件可收敛为兼容层或迁移原型。
