...
本模块是 extras 的 client 锚点包：把 gates/prompt 各模块的 Settings Tab 组合成单一浏览器 bundle，并通过自有嵌套 manifest 使这个合成 bundle 成为恰好一个 Loader source。包内角色：extras 根保持 server-only（不声明 `dsh.client`），全部 client 面集中在本目录。
