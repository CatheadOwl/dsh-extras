...
## Conventions（rules SSOT）

本节是包独立性目标的 **rules SSOT seed**，双端消费：生成时（本文件被 agent
自动加载）控制写作；评审时（review 技能的 dispatch prompt **原样嵌入**本节，
finding 引用 rule id）。规则只写期望形态；理由归认知层/决策史。id 一经评审/
gate 引用即不改号，作废标「已废弃」不复用。
